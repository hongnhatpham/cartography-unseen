...
class Camera:
    ...
    def create_default(cls) -> "Camera":
        return cls(position=np.array([0.0, 1.65, 5.5], dtype=np.float32))

    def reset(self) -> None:
        self.position[:] = (0.0, 1.65, 5.5)
        self.yaw = 0.0
        self.pitch = 0.0
    ...
    def forward(self) -> np.ndarray:
        ...
        return np.array(
            [sin(yaw) * cos(pitch), sin(pitch), -cos(yaw) * cos(pitch)], dtype=np.float32
        )

    @property
    def flat_forward(self) -> np.ndarray:
        yaw = radians(self.yaw)
        return np.array([sin(yaw), 0.0, -cos(yaw)], dtype=np.float32)

    @property
    def right(self) -> np.ndarray:
        yaw = radians(self.yaw)
        return np.array([cos(yaw), 0.0, sin(yaw)], dtype=np.float32)
    ...
    def move(self, local_x: float, local_z: float, vertical: float, distance: float) -> None:
